...
`addappt` Command - Adds an appointment or recursive appointments to a patient.
...
The `missAppointment` command displays the miseed appointments. The missed appointments are found in `ModelManager`. The list is instantiated by filtering the `UniqueAppointmentList` using `EventsMissedPredicate` which checks all `APPROVED`-Status appointments ending times with current time.
